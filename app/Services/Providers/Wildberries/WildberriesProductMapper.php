 ...
 * ExternalProductData.
 *
 * Payload contract (one dict per product card, snake_case, built by
 * `docker/playwright/scrapers/wildberries.js`):
 *
 ...
 * currency as an ISO string. A scraped page can always be incomplete — a card
 * may lack a price, a rating or an image — so every access is null-tolerant
 * and an unusable card is dropped rather than raising.
 */
class WildberriesProductMapper
{
    /**
     * Map a whole scrape response. Cards without a title are dropped: they are
     * promo/banner slots rather than products.
     *
     * The query is accepted for symmetry with the other providers; no local
     * filtering happens here, because Wildberries' own search engine already
     * answered the text query and re-filtering by substring would discard
     * legitimately relevant results.
     *
    ...
    /**
     * The scraper recovers the numeric WB article id from the card link; when
     * the card hides it, a stable hash of the product URL keeps de-duplication
     * and the offer's database identity working across searches.
     *
     * @param array<string, mixed> $raw
     */
    private function externalProductId(array $raw): string
    {
        ...
        return 'wb:' . md5((string) ($raw['product_url'] ?? $raw['title'] ?? ''));
    }

    /**
     * Prices are already integer kopecks; anything non-numeric or non-positive
     * means "the card did not show a price".
     */
    ...
    /**
     * A card carries a single thumbnail; the DTO keeps a list so the product
     ...
     * normalised card itself so the payload is never empty for debugging.
     *
     * @param array<string, mixed> $raw

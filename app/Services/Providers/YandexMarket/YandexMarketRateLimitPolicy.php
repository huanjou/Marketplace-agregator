...
/**
 * Guards the Yandex Market Partner API quota with a per-minute sliding window
 * shared by every worker through the cache store.
 */
class YandexMarketRateLimitPolicy
{
    private const KEY = 'provider:yandex_market:rate';

    private const DECAY_SECONDS = 60;

    private const DEFAULT_LIMIT_PER_MINUTE = 100;
    ...
    public function attempt(callable $callback): mixed
    {
        ...
        if ($executed === false) {
            throw new ProviderRateLimitException(
                ProviderCode::YandexMarket->value,
                RateLimiter::availableIn(self::KEY),
                sprintf('Local Yandex Market quota of %d requests per minute exhausted.', $limit),
            );
        }
    ...
    private function limitPerMinute(): int
    {
        return (int) Config::get(
            'marketplace.providers.yandex_market.rate_limit_per_minute',
